...
def create_model():
    ...
    # generate to connect between the frieze model to the output layer
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    x = tf.keras.layers.Dense(1024, activation='relu')(x)
    predictions = tf.keras.layers.Dense(label_names_len, activation='softmax')(x)
    resnet_model = tf.keras.Model(inputs=restnet.input, outputs=predictions)

    # frieze the pretrain model
